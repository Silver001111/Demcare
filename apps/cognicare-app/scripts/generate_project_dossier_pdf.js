...
function addSectionHeader(title, subtitle = null) {
  ...
  doc.text(title, MARGIN_LEFT + 7, curY + 7);
  curY += 11;
...
function addParagraph(text, spacing = 2) {
  ...
  checkPageBreak(lines.length * 4.2 + spacing);
  doc.text(lines, MARGIN_LEFT, curY);
  curY += lines.length * 4.2 + spacing;
}
...
function addBullet(title, text) {
  ...
  doc.circle(MARGIN_LEFT + 2, curY - 1.2, 1, 'F');

  doc.setFont('helvetica', 'bold');
  doc.setFontSize(9);
  doc.setTextColor(FOREST_DARK[0], FOREST_DARK[1], FOREST_DARK[2]);
  doc.text(title + ':', MARGIN_LEFT + 6, curY);
  
  const titleWidth = doc.getTextWidth(title + ': ');
  doc.setFont('helvetica', 'normal');
  doc.setTextColor(BARK_TEXT[0], BARK_TEXT[1], BARK_TEXT[2]);
  
  // if fits on first line
  if (lines.length === 1) {
    doc.text(text, MARGIN_LEFT + 6 + titleWidth, curY);
    curY += 4.5;
  } else {
    // split text specifically after title
    const wrappedLines = doc.splitTextToSize(text, CONTENT_WIDTH - 6 - titleWidth);
    doc.text(wrappedLines[0], MARGIN_LEFT + 6 + titleWidth, curY);
    curY += 4.2;
    const remainingLines = doc.splitTextToSize(text.substring(wrappedLines[0].length).trim(), CONTENT_WIDTH - 6);
    if (remainingLines.length > 0 && remainingLines[0] !== '') {
      doc.text(remainingLines, MARGIN_LEFT + 6, curY);
      curY += remainingLines.length * 4.2;
    }
    curY += 1.5;
  }
}
...
function addCalloutBox(title, text, type = 'emerald') {
  ...
  const boxHeight = 8 + textLines.length * 4 + 4;
  ...
  doc.text(title, MARGIN_LEFT + 6, curY + 5.5);
  
  // Text
  doc.setFont('helvetica', 'normal');
  doc.setFontSize(8.5);
  doc.setTextColor(BARK_TEXT[0], BARK_TEXT[1], BARK_TEXT[2]);
  doc.text(textLines, MARGIN_LEFT + 6, curY + 10.5);
...
function addTable(headers, rows, colWidths) {
  ...
  for (let i = 0; i < headers.length; i++) {
    doc.text(headers[i], curX + 2, curY + 4.8);
    curX += colWidths[i];
  }
  curY += headerHeight;
  ...
  for (let r = 0; r < rows.length; r++) {
    ...
    for (let c = 0; c < rows[r].length; c++) {
      ...
      doc.text(String(rows[r][c]), curX + 2, curY + 4.5);
      curX += colWidths[c];
    }
    curY += rowHeight;
  }
...
function buildCoverPage() {
  ...
  doc.text('SMART INDIA HACKATHON 2026 • OFFICIAL MASTER TECHNICAL DOSSIER', 16, 22);
  
  // App Title
  doc.setFont('helvetica', 'bold');
  doc.setFontSize(26);
  doc.setTextColor(255, 255, 255);
  doc.text('CogniCare NER (স্মৃতি-সেতু)', 16, 36);
  
  // Subtitle
  doc.setFont('helvetica', 'normal');
  doc.setFontSize(11.5);
  doc.setTextColor(220, 240, 230);
  doc.text('Offline-First, Culturally-Grounded Multilingual AI Digital Therapeutic', 16, 45);
  doc.text('& Clinical Surveillance Ecosystem for Geriatric Cognitive Care', 16, 52);
  
  // Institutional Endorsement Bar
  doc.setFont('helvetica', 'italic');
  doc.setFontSize(9.5);
  doc.setTextColor(190, 215, 205);
  doc.text('Designed for: Ministry of Development of North Eastern Region (MDoNER) & LGBRIMH Tezpur', 16, 66);
  
  // Body Content on Cover Page
  curY = 92;
...
function buildAllContentPages() {
  ...
  // SECTION 1: PROBLEM CONTEXT & NEED IN NORTH EAST INDIA
  addSectionHeader(
  ...
  // SECTION 3: MULTILINGUAL FOUNDATION & DIALECT PARSER
  addSectionHeader(
  ...
  // SECTION 4: THE 8 NEURO-THERAPEUTIC WORKOUTS
  addSectionHeader(
  ...
  addParagraph(
    'Outputs standard Rouleau Scale (1 to 5) and Sunderland Composite Score (1 to 10) for direct clinical review.'
  );
  ...
  // SECTION 5: PATIENT SAFETY, BPSD & ASSISTIVE CARE
  addSectionHeader(
  ...
  // SECTION 6: CAREGIVER DASHBOARD & PREDICTIVE INTELLIGENCE
  addSectionHeader(
  ...
  // SECTION 7: ASHA WORKER GATEWAY & ZERO-INTERNET P2P BATCH SYNC
  addSectionHeader(
  ...
  // SECTION 9: DATA PRIVACY & CLINICAL VALIDATION
  addSectionHeader(
  ...
  // SECTION 10: CONCLUSION & FUTURE ROADMAP
  addSectionHeader(
...
const outputPath = path.join(outputDir, 'COGNICARE_NER_MASTER_SYSTEM_DOSSIER.pdf');
...
console.log('File Size (KB):', (fs.statSync(outputPath).size / 1024).toFixed(1));
